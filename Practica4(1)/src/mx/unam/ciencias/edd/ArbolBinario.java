...
public abstract class ArbolBinario<T> implements Coleccion<T> {
    ...
    private int getProfundidad(Vertice vertice){
        if ( this.elementos == 1 || vertice == null )
            return 0;
        if ( vertice.equals( this.raiz ) )
            return ( int ) Math.max(getProfundidad(vertice.izquierdo),(getProfundidad(vertice.derecho ) ) );
        return ( int )Math.max(getProfundidad(vertice.izquierdo),(getProfundidad(vertice.derecho) ) ) + 1;
    }
